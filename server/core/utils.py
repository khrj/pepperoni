...
def categorize_delays(packets):
    ...
    for i, packet in enumerate(packets):
        if i > 0:
            ...
            if protocol == "MQTT" and packet.get("mqtt_type") in [3, 4]:  # PUBLISH and PUBACK
...
def get_insights_data(packets):
    ...
    for i, packet in enumerate(sorted_packets):
        ...
        if delay > 0:
            ...
            if protocol == "TCP" and packet.get("is_retransmission", False):
                retransmissions.append({"timestamp": packet["timestamp"], "delay": delay})
    ...
    if retransmissions:
        ...
        spike_periods = [time for time, pkts in retrans_by_time.items() if len(pkts) > 5]
...
def get_packet_data(packets, limit=100):
    ...
    for packet in sorted_packets:
        ...
        delay = int(size / 10) + (50 if protocol == "MQTT" else 30)  # Simplified delay estimate
